    ...
    try {
        switch (name) {
            ...
            case 'update_location': {
                const { location_id, lat, lng, radius_meters } = args;
                const updates = {};
                if (lat !== undefined)
                    updates.lat = lat;
                if (lng !== undefined)
            ...
            case 'add_teacher': {
                const { teacher_id, full_name, position, location_id, pin_code, is_admin } = args;
                const { data, error } = await supabase
                    .from('teachers')
                    .insert({
                    teacher_id,
            ...
            case 'list_attendance_records': {
                const { date, location_id, teacher_id, limit } = args;
                let query = supabase
            ...
            case 'update_settings': {
                const { check_in_start, check_in_end, check_out_start, check_out_end } = args;
                const updates = { updated_at: new Date().toISOString() };
                if (check_in_start)
                    updates.check_in_start = check_in_start;
                if (check_in_end)
                    updates.check_in_end = check_in_end;
                if (check_out_start)
                    updates.check_out_start = check_out_start;
                if (check_out_end)
                    updates.check_out_end = check_out_end;
                const { data, error } = await supabase
